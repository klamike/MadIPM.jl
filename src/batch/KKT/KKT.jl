# ============================================================================
# Batch KKT system — abstract top + shared `_kktmul!` for unreduced systems.
# ============================================================================

abstract type AbstractBatchKKTSystem{T} end

@inbounds function _kktmul!(
    w::BatchUnreducedKKTVector, x::BatchUnreducedKKTVector,
    ...
    MadNLP.dual_lb(w)  .= beta .* MadNLP.dual_lb(w) .+
                          alpha .* (xp_lr(x) .* l_lower .- MadNLP.dual_lb(x) .* l_diag)
    MadNLP.dual_ub(w)  .= beta .* MadNLP.dual_ub(w) .+
                          alpha .* (xp_ur(x) .* u_lower .+ MadNLP.dual_ub(x) .* u_diag)
    return nothing
end
